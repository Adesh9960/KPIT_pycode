...
import sys
import requests
from Data_generation import actuators_logic as actuators
from Data_generation.display_data import display_stats
from Data_generation.UDSHandler import UDSHandler
...
def run_vehicle_simulator():
    ...
    while True:
        ...
        # 2. Dynamically map your LIVE telemetry DID integers to telemetry_row keys.
        live_dids_payload = {}
        did_telemetry_map = {
            0xF401: ("Speed_kmh", int),
            0xF402: ("Engine_RPM", int),
            0xF403: ("Coolant_Temp_C", int),
            0xF404: ("Oil_Temp_C", int),
            0xF405: ("Fuel_Pct", int),
            0xF406: ("Battery_V", float),
            0xF408: ("Throttle_Pct", int)
        }
        
        for did_int, (dict_key, cast_type) in did_telemetry_map.items():
            did_hex_str = f"0x{did_int:04X}"
            live_dids_payload[did_hex_str] = cast_type(telemetry_row.get(dict_key, 0))
        ...
        payload = {
            ...
            "speed": float(telemetry_row.get("Speed_kmh", 0)),
            "rpm": int(telemetry_row.get("Engine_RPM", 0)),
            "coolant": float(telemetry_row.get("Coolant_Temp_C", 0)),
            "coolant_temp": float(telemetry_row.get("Coolant_Temp_C", 0)),
            "oil_temp": float(telemetry_row.get("Oil_Temp_C", 0)),
            "ambient_temp": float(telemetry_row.get("Ambient_Temp_C", 0)),
            "fuel": float(telemetry_row.get("Fuel_Pct", 0)),
            "fuel_pct": float(telemetry_row.get("Fuel_Pct", 0)),
            "fuel_rate": float(telemetry_row.get("Fuel_Rate_mL_s", 0)),
            "remaining_fuel_l": float(telemetry_row.get("Remaining_Fuel_L", 0)),
            "distance_km": float(telemetry_row.get("Distance_km", 0)),
            "accel": float(telemetry_row.get("Accel_ms2", 0)),
            "accel_ms2": float(telemetry_row.get("Accel_ms2", 0)),
            "engine_load": float(telemetry_row.get("Engine_Load_Pct", 0)),
            ...
            "voltage": float(telemetry_row.get("Battery_V", 0)),

            # --- DYNAMIC didList.py INJECTION ---
            "live_dids": live_dids_payload
        }

        # 4. Transmit the data directly to app.py's RAM
        try:
            requests.post(
                "http://127.0.0.1:9000/internal/update_telemetry", 
                json=payload, 
                timeout=0.05
            )
        ...
        # ═══════════════════════════════════════════════════════════════
        #  FETCH ACTUATOR OVERRIDES FROM FLASK (TWO-WAY IPC)
        # ═══════════════════════════════════════════════════════════════
        try:
            # Poll Flask for any active UDS 0x2F IO Controls
            override_resp = requests.get("http://127.0.0.1:9000/internal/get_overrides", timeout=0.02)
            if override_resp.status_code == 200:
                overrides = override_resp.json()
                from actuators.actuator import Control # Ensure this is imported at the top
                
                # DID 0xF410 (62480) = Head Lamp
                if "62480" in overrides:
                    main.head_lamp.control = Control(overrides["62480"]["control"])
                    main.head_lamp.state = overrides["62480"]["state"]
                    
                # DID 0xF411 (62481) = Radiator Fan
                if "62481" in overrides:
                    main.radiator_fan.control = Control(overrides["62481"]["control"])
                    main.radiator_fan.state = overrides["62481"]["state"]
                    
                # DID 0xF412 (62482) = Fuel Pump
                if "62482" in overrides:
                    main.fuel_pump.control = Control(overrides["62482"]["control"])
                    main.fuel_pump.state = overrides["62482"]["state"]
                    
        except requests.exceptions.RequestException:
            # Silently ignore if Flask is offline so the engine keeps running
            pass
